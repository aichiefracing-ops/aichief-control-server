...
STRIPE_PRO_PLUS_IDS = [
    "prod_U1OeXZPAcV8j3p",     # legacy Pro+ product (kept so existing subs still resolve)
    "prod_U1OkjYcecOg7Gz",     # legacy Pro+ product (kept so existing subs still resolve)
    "prod_U3xVN0fGmn9YXp",     # Pro+ Monthly $19.99 — current buy link
    "prod_U3xVux3fJNSHo7",     # Pro+ Yearly $199.99 — current buy link
]
    ...
    try:
        ...
        for customer in customers:
            ...
            subs_r = requests.get(
                ...
                params={"customer": cid, "status": "all", "limit": 10, "expand[]": "data.discount.promotion_code"},
                auth=(STRIPE_SECRET_KEY, ""),
                timeout=8,
            )
            if not subs_r.ok:
            ...
            for sub in subs:
                # status=all returns every sub incl. canceled/incomplete. Only these
                # are entitled: active (paid & current), trialing (in a free-trial
                # window — the 7-day Pro+ trial lives here), past_due (failed-renewal
                # grace). Skip the rest so a canceled/never-paid sub can't unlock.
                ...
                if r.ok:
                    for customer in r.json().get("data", []):
                        ...
                        sr = requests.get("https://api.stripe.com/v1/subscriptions",
                                          params={"customer": cid, "status": "all", "limit": 10},
                                          auth=(STRIPE_SECRET_KEY, ""), timeout=8)
                        if not sr.ok:
                            continue
                        for sub in sr.json().get("data", []):
                            # entitled statuses only (incl. trialing for the Pro+ trial);
                            # skip canceled/incomplete/unpaid so they can't unlock a room.
                            if sub.get("status") not in ("active", "trialing", "past_due"):
                                continue
                            for item in sub.get("items", {}).get("data", []):
...
def admin_team_rooms(
    ...
    with _TR_LOCK:
        ...
        return {"ok": True, "enforce": TEAM_RELAY_ENFORCE, "rooms": rooms}        if not _PG_URL:
            return
        try:
            with _pg_conn() as conn:
                with conn.cursor() as cur:
